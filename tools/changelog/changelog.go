...
var (
	...
	webconsoleBump = regexp.MustCompile(regexp.QuoteMeta("bump(github.com/enj/example-operator-web-console): ") + `([\w]+)`)
...
func main() {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		// has api changes
		display := fmt.Sprintf("%s [\\#%s](https://github.com/enj/example-operator/pull/%s)", message, matches[1], matches[1])
